...
import { Search, UserPlus, Phone, Mail, ChevronRight, UserX } from 'lucide-react'
    ...
    <AppLayout companyName={companyName} user={{ name: user?.user_metadata?.name, email: user?.email }}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative">
        ...
        <form className="mb-6 relative animate-in fade-in slide-in-from-bottom-2 duration-300">
          <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-purple-200/50" />
          <input
            type="search"
            name="q"
        ...
        {loading ? (
          ...
          <GlassCard variant="default" className="p-12 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
            <AnimatedIcon icon={UserX} variant="scale" size={48} className="text-fuchsia-400 mb-4 mx-auto" />
                ...
                <Link href={`/app/clientes/${client.id}`}>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      {/* Arrow */}
                      <ChevronRight size={20} className="text-purple-200/40 group-hover:text-purple-200 group-hover:translate-x-1 transition-all flex-shrink-0" />
                    </div>
                  </GlassCard>
                </Link>
              </motion.div>
